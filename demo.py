...
def draw_registration_result(src_raw, tgt_raw, src_index, tgt_index, tsfm, filename):
    ########################################
    # 1. input point cloud
...
def main(config, demo_loader, model):
    ...
    with torch.no_grad():
        for batch_idx in track(range(tot_num_data)):
            ...
            sname, xyz0, xyz1, trans, filename = batch[0]
            ...
            10_758_765
            if '10' and '758' and '765' in filename:
                draw_registration_result(xyz0, xyz1, over_index0, over_index1, T, filename)

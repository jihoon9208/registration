...
from model.helpers import GenericMLP
...
class PointTransfRef(nn.Module):
  def __init__(self, block=PointTransformerBlock, blocks=1, c=7, out_planes=32, share_planes=8, nsample=16, enc_dim=1):
    super().__init__()
    self.c = c
  ...
  def forward(self, trans_coord, trans_feat=None):
    
    npts = trans_coord.shape[0]
    p0 = input.view(-1, self.c).clone()
    x0 = input.view(-1, self.c).clone()
    o0 = torch.IntTensor([npts*(i+1) for i in range(bsize)]).cuda()

    p2, x2, o2 = self.enc1([p0, trans_feat, o0])    # t0 128 channels
